...
function VideoProjects ({obj}) {
  ...
  return(
    <div class={`${styles.container} ${styles.cp}`}>
      ...
      {obj.lugar && (<p><span class={styles.bold}>Local:</span> {obj.lugar}</p>)}
      {obj.cargo && (<p><span class={styles.bold}>Cargo:</span> {obj.cargo}</p>   )}   
      {obj.equipo && (
        <>
      ...
      {obj.misiones && (<p><span class={styles.bold}>Missões:</span> {obj.misiones}</p>)}
      ...
      {obj.aforo && (<p><span class={styles.bold}>Público:</span> {obj.aforo}</p>)}
      {obj.reconocimientos && (<p><span class={styles.bold}>Reconhecimentos:</span> {obj.reconocimientos}</p>)}
      {obj.apoyo && (<p><span class={styles.bold}>Com o apoio de:</span> {obj.apoyo}</p>)}
      {obj.enlaces && (
        <>
  ...
  return(
    <div class={styles.container}>
      <ul>
        ...
        <li id="sucursalfest_" class={styles.proyecto} onclick={open_SucursalFest}>Astronativo no Sucursal Fest <span class={styles.type}>Show - Visuais</span> / 2025</li>
        {showSucursalFest() && (<VideoProjects obj={videosApi.sucursalfest}/>)}

        <li id="sbiodiversidad" class={styles.proyecto} onclick={open_SemanaBio}>Astronativo na Semana da Biodiversidade <span class={styles.type}>Show - Visuais</span> / 2025</li>
        {showSemanaBio() && (<VideoProjects obj={videosApi.biodiversidad}/>)}

        <li id="cop16_" class={styles.proyecto} onclick={open_Cop16}>Astronaitvo na COP16 <span class={styles.type}>Show - Visuais</span> / 2024</li>
        {showCop16() && (<VideoProjects obj={videosApi.cop16}/>)}                

        <li id="rioradio" class={styles.proyecto} onclick={open_RioRadio}>Río/Radio: Correspondência anfíbia <span class={styles.type}>AV Performance</span> / 2021</li>
        ...
        <li id="derkirschgarten" class={styles.proyecto} onclick={open_DerKirchgarten}>Der Kirschgarten <span class={styles.type}>Teatro Contemporâneo</span> / 2018</li>
        {showDerKirchgarten() && (<VideoProjects obj={videosApi.der_kirchgarten}/>)}
          
        <li id="dieoriginale" class={styles.proyecto} onclick={open_DieOriginale}>Die Originale <span class={styles.type}>Circo Contemporâneo</span> / 2018</li>
        ...
        <li id="avinstall" class={styles.proyecto} onclick={open_AvInstall}>AV Instalation <span class={styles.type}>Instalação Interativa</span> / 2017</li>
        ...
        <li id="guayabobros" class={styles.proyecto} onclick={open_GuayaBros}>Guayabo Borthers <span class={styles.type}>Show - Visuais</span> / 2017-2018</li>
        ...
        <li id="imputaciones" class={styles.proyecto} onclick={open_Imputaciones}>Imputaciones <span class={styles.type}>Cabaret Político Multimídia</span> / 2016</li>
        {showImputaciones() && (<VideoProjects obj={videosApi.imputaciones}/>)}
        
        <li id="esteropicnic" class={styles.proyecto} onclick={open_EsteroPicnic}>Electric Mistakes: Estero Picnic <span class={styles.type}>Show - Visuais</span> / 2016</li>
        {showEsteroPicnic() && (<VideoProjects obj={videosApi.esteropicnic} />)}
        
        <li id="famiasuto" class={styles.proyecto} onclick={open_FamiaSuto}>Famia Suto: Hip-Hop al Parque <span class={styles.type}>Show - Visuais</span> / 2015</li>
        ...
        <li id="atusombra" class={styles.proyecto} onclick={open_Atusombra}>A tu sombra <span class={styles.type}>Teatro Multimídia</span> / 2012</li>
